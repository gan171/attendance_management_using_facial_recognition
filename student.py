...
class Student:
    ...
    # This part is image labels setting start 
        ...
        dep_combo["values"]=("Select Department","CS","IT","STATS","PHY","MATH","CHEM")
        dep_combo.current(0)
        ...
        cou_combo["values"]=("Select Course","FY","SY","TY")
        cou_combo.current(0)
        ...
        year_combo["values"]=("Select Year","2020-23","2021-24","2022-25","2023-26","2024-27")
...
# ==================Function Decleration==============================
    def add_data(self):
        email_inp = self.var_email.get()
        regex = re.compile(r'([A-Za-z0-9]+[.-_])*[A-Za-z0-9]+@[A-Za-z0-9-]+(\.[A-Z|a-z]{2,})+')
        if re.match(regex,email_inp):
            pass
        else:
            messagebox.showerror("Error","Invalid E-mail address",parent=self.root)
            mainloop.destroy()
            

        mob_num =self.var_mob.get()
        mob_validate=re.fullmatch('[6-9][0-9]{9}',mob_num) # calling fullmatch function by passing pattern and n
        if mob_validate!=None:
            pass
        else:
    ...
    def delete_data(self):
        print('here')
        (os.remove(file) for file in os.listdir(r'C:\Users\91970\Desktop\facerec33\Python_Test_Projects\data_img') if file.endswith(''+self.var_std_id+'.jpg'))
    ...
    def generate_dataset(self):
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            ...
            try:
                ...
                face_classifier = cv2.CascadeClassifier(r"C:\Users\91970\Desktop\facerec33\Python_Test_Projects\haarcascade_frontalface_default.xml")
                ...
                while True:
                    ...
                    if face_croped(my_frame) is not None:
                        ...
                        print(face)
                        print("We here.....")
                        file_path=r"C:/Users/91970/Desktop/facerec33/Python_Test_Projects/data_img/student."+str(id)+"."+str(img_id)+".jpg"
                        print(file_path)
                        cv2.imwrite(file_path,face)
                        a = cv2.putText(face,str(img_id),(50,50),cv2.FONT_HERSHEY_COMPLEX,2,(0,255,0),2)    
                        print(a)    
                        b = cv2.imshow("Capture Images",face)
                        print(b)
